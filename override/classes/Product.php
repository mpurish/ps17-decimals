...
class ProductCore extends ObjectModel
{
    ...
    */
    ...
    public function addAttribute(
        ...
        $combination->price = (float)$price;
        $combination->ecotax = (float)$ecotax;
        $combination->quantity = (float)$quantity;
        $combination->weight = (float)$weight;
    ...
    */
    ...
    public static function priceCalculation(
        ...
        $cache_id = (int)$id_product.'-'.(int)$id_shop.'-'.(int)$id_currency.'-'.(int)$id_country.'-'.$id_state.'-'.$zipcode.'-'.(int)$id_group.
            '-'.(float)$quantity.'-'.(int)$id_product_attribute.'-'.(int)$id_customization.
    ...
    */
    ...
    public static function getProductProperties($id_lang, $row, Context $context = null)
    {
        ...
        if (isset($row['quantity_wanted'])) {
            ...
            $quantity = max((float)$row['minimal_quantity'], (float)$row['quantity_wanted']);
        } else {
            $quantity = (float)$row['minimal_quantity'];
        }
    ...
    */
    ...
    public static function getAllCustomizedDatas($id_cart, $id_lang = null, $only_in_cart = true, $id_shop = null, $id_customization = null)
    {
        ...
        foreach ($result as $row) {
            $customized_datas[(int)$row['id_product']][(int)$row['id_product_attribute']][(int)$row['id_address_delivery']][(int)$row['id_customization']]['quantity'] = (float)$row['quantity'];
            $customized_datas[(int)$row['id_product']][(int)$row['id_product_attribute']][(int)$row['id_address_delivery']][(int)$row['id_customization']]['quantity_refunded'] = (float)$row['quantity_refunded'];
            $customized_datas[(int)$row['id_product']][(int)$row['id_product_attribute']][(int)$row['id_address_delivery']][(int)$row['id_customization']]['quantity_returned'] = (float)$row['quantity_returned'];
    ...
    public static function addCustomizationPrice(&$products, &$customized_datas)
    {
        ...
        foreach ($products as &$product_update) {
            if (!Customization::isFeatureActive()) {
                ...
                $product_quantity = isset($product_update['cart_quantity']) ? (float)$product_update['cart_quantity'] : (float)$product_update['product_quantity'];
                ...
                if (isset($customized_datas[$product_id][$product_attribute_id][$id_address_delivery])) {
                    foreach ($customized_datas[$product_id][$product_attribute_id][$id_address_delivery] as $customization) {
                        ...
                        $customization_quantity_refunded += (float)$customization['quantity_refunded'];
                        $customization_quantity_returned += (float)$customization['quantity_returned'];
                    }
                }
    ...
    */
    ...
    public function addStockMvt($quantity, $id_reason, $id_product_attribute = null, $id_order = null, $id_employee = null)
    {
        ...
        $quantity = abs((float)$quantity) * $reason->sign;
    ...
    */
    ...
    public function setWsProductBundle($items)
    {
        ...
        foreach ($items as $item) {
            if ((int)$item['id'] > 0) {
                Pack::addItem($this->id, (int)$item['id'], (float)$item['quantity']);
            }
        }
        return true;
    }
