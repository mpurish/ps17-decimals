    ...
    */
    ...
    public function deleteProduct($order, $order_detail, $quantity)
    {
        ...
        if ($this->hasBeenDelivered()) {
            ...
            $order_detail->product_quantity_return += (float)$quantity;
            return $order_detail->update();
        } elseif ($this->hasBeenPaid()) {
            $order_detail->product_quantity_refunded += (float)$quantity;
            return $order_detail->update();
        }
        return $this->_deleteProduct($order_detail, (float)$quantity);
    }
    ...
    protected function _deleteProduct($order_detail, $quantity)
    {
        ...
        /* Update order detail */
        $order_detail->product_quantity -= (float)$quantity;
    ...
    public function deleteCustomization($id_customization, $quantity, $order_detail)
    {
        ...
        if ($this->hasBeenDelivered()) {
            return Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customization` SET `quantity_returned` = `quantity_returned` + '.(float)$quantity.' WHERE `id_customization` = '.(int)$id_customization.' AND `id_cart` = '.(int)$this->id_cart.' AND `id_product` = '.(int)$order_detail->product_id);
        } elseif ($this->hasBeenPaid()) {
            return Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customization` SET `quantity_refunded` = `quantity_refunded` + '.(float)$quantity.' WHERE `id_customization` = '.(int)$id_customization.' AND `id_cart` = '.(int)$this->id_cart.' AND `id_product` = '.(int)$order_detail->product_id);
        }
        if (!Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customization` SET `quantity` = `quantity` - '.(float)$quantity.' WHERE `id_customization` = '.(int)$id_customization.' AND `id_cart` = '.(int)$this->id_cart.' AND `id_product` = '.(int)$order_detail->product_id)) {
            return false;
        }
            ...
            if ($selected_qty) {
                ...
                foreach ($selected_products as $key => $id_product) {
                    if ($row['id_order_detail'] == $id_product) {
                        $row['product_quantity'] = (float)$selected_qty[$key];
                    }
                }
