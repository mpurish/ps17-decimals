...
class OrderDetailCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'product_quantity' =>            array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat', 'required' => true),
            'product_quantity_in_stock' =>    array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
            'product_quantity_return' =>    array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
            'product_quantity_refunded' =>    array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
            'product_quantity_reinjected' =>array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
    ...
    protected function checkProductStock($product, $id_order_state)
    {
        if ($id_order_state != Configuration::get('PS_OS_CANCELED') && $id_order_state != Configuration::get('PS_OS_ERROR')) {
            ...
            if (!StockAvailable::dependsOnStock($product['id_product'])) {
                $update_quantity = StockAvailable::updateQuantity($product['id_product'], $product['id_product_attribute'], -(float)$product['cart_quantity']);
            }
    ...
    protected function create(Order $order, Cart $cart, $product, $id_order_state, $id_order_invoice, $use_taxes = true, $id_warehouse = 0)
    {
        ...
        $product_quantity = (float)Product::getQuantity($this->product_id, $this->product_attribute_id, null, $cart);
        $this->product_quantity_in_stock = ($product_quantity - (float)$product['cart_quantity'] < 0) ?
            $product_quantity : (float)$product['cart_quantity'];
