...
class CartCore extends ObjectModel
{
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null, $fullInfos = true)
    {
        ...
        if ($fullInfos) {
            ...
            foreach ($result as &$row) {
                ...
                if (!$row['is_gift'] || (float)$row['cart_quantity'] === $givenAwayQuantity) {
    ...
    protected function applyProductCalculations($row, $shopContext, $productQuantity = null)
    {
        if (is_null($productQuantity)) {
            $productQuantity = (float)$row['cart_quantity'];
        }
    ...
    public static function getNbProducts($id)
    {
        ...
        self::$_nbProducts[$id] = (float)Db::getInstance()->getValue(
            'SELECT SUM(`quantity`)
            FROM `'._DB_PREFIX_.'cart_product`
     ...
     *
     * @param float    $quantity             Quantity to add (or substract)
     * @param int    $id_product           Product ID
    ...
    public function updateQty(
        ...
        if (!empty($id_product_attribute)) {
            $minimal_quantity = (float)Attribute::getAttributeMinimalQty($id_product_attribute);
        } else {
            $minimal_quantity = (float)$product->minimal_quantity;
        }
    ...
    public function duplicate()
    {
        ...
        foreach ($customs_by_id as $customization_id => $val) {
            ...
            Db::getInstance()->execute(
                ...
                VALUES('.(int)$cart->id.', '.(int)$val['id_product_attribute'].', '.(int)$val['id_product'].', '.(int)$id_address_delivery.', '.(float)$val['quantity'].', 0, 0, 1)'
            );
    ...
    public function setProductAddressDelivery($id_product, $id_product_attribute, $old_id_address_delivery, $new_id_address_delivery)
    {
        ...
        $sql = 'UPDATE '._DB_PREFIX_.'cart_product
            ...
            `quantity` = `quantity` + '.(float)$result.'
            WHERE id_product = '.(int)$id_product.'
            AND id_product_attribute = '.(int)$id_product_attribute.'
    ...
    public function duplicateProduct(
        ...
        if ($customization_count > 0) {
            $sql = 'UPDATE '._DB_PREFIX_.'cart_product
                SET `quantity` = `quantity` + '.(float)$customization_count * $quantity.'
                WHERE id_cart = '.(int)$this->id.'
                AND id_product = '.(int)$id_product.'
                AND id_shop = '.(int)$this->id_shop.'
                AND id_product_attribute = '.(int)$id_product_attribute.'
