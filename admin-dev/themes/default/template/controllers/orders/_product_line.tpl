...
<tr class="product-line-row">
	...
	{if ((!$order->hasBeenDelivered() OR Configuration::get('PS_ORDER_RETURN')) AND (float)($product['product_quantity_return']) < (float)($product['product_quantity']))}
                ...
                {if !empty($pack_item.active)}
                    <tr class="product-line-row" {if isset($pack_item.image) && $pack_item.image->id && isset($pack_item.image_size)} height="{$pack_item['image_size'][1] + 7}"{/if}>
                        ...
                        <td class="productQuantity">
                            <span class="product_quantity_show{if (float)$pack_item.pack_quantity > 1} red bold{/if}">{$pack_item.pack_quantity}</span>
                        </td>
